...
import pandas as pd
genai.configure(api_key="AIzaSyBjT_lfZk9v3OrgQPvQiOJwhfcmq2ymu_Y")
...
def extract_average_package(review):
    patient_count_prompt = f"""Given a paragraph, return average package in integer format, it may be in LPA or lakh or thousand or any other format, return proper integer (return zero if found none) as a single number in response, for example: if given 12LPA return 1200000\n\nparagraph:\n{review}"""
...
if col1.button("Generate Score"):
    ...
    if 200 <= review_length <= 500:
        ...
        if rev_pat != 0 and abs(rev_pat - nirf_pat) > 2:
            ...
            st.success("No issues with patents")
        ########################################
        time.sleep(3)
        ...
        if rev_pak != 0 and abs(rev_pak - nirf_pak) > 150000:
            ...
            st.success("No issues with packages")
        ########################################
        time.sleep(3)
        ...
        if rev_cor != 0 and abs(rev_cor - nirf_cor) > 2:
            ...
            st.success("No issues with corses")
        ########################################
        time.sleep(3)
        rev_add = extract_addmission(user_input)
        nirf_add = record['addmissions'].iloc[0]

        if rev_add != 0 and abs(rev_add - nirf_add) > 10:
            ...
            st.success("No issues with addmissions")
        ########################################

        if sucess:
            st.success("Your NIRF data matches with review")
